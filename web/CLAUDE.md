...
DEV is a local `npm run dev` with `USE_EXTERNAL_API=true` by default (see `.env.local.example`) — it calls a locally-running `eduverify-api` dev server instead of the bundled local-seed/DynamoDB path, so this repo doesn't need to hold its own copy of institution/qualification state day-to-day. Start `eduverify-api` first (`npm run dynamodb:local`, `npm run seed:local`, then `PORT=4000 npm run dev` in that repo — its default port 3000 collides with this app's own `next dev`). Set `USE_EXTERNAL_API=false` locally to fall back to the bundled seed data (e.g. for offline work, or when `eduverify-api` isn't running — there's no automatic fallback if the flag is `true` and the API is unreachable). STAGING (the `staging` branch) also sets `USE_EXTERNAL_API=true`, against `eduverify-api-staging`. PROD (`main`) stays `false` until the parity check against `eduverify-api-prod` passes (`web/scripts/parityCheck.ts`). See `docs/DEPLOYMENT.md`'s "DEV, and the branch → environment flow" for the full feature-branch → staging → main promotion path and why direct pushes to `main`/`staging` are rejected.
