...
└── terraform/   AWS infra for web/ — DynamoDB (read side), Amplify hosting, powered by scripts/
```

Scraping the DHET PDF/SAQA spreadsheet and writing the live DynamoDB table is
**not** this repo's job — that ingestion pipeline lives entirely in the
sibling `eduverify-api` repo. This repo just reads what it produces, plus
keeps `data/institutions.json` and `data/qualifications.json` committed as
static bundled fixtures. As a new engineer, **you will spend almost all your
time in `web/`.** You should not touch `terraform/` without a senior engineer
pairing with you — it manages real AWS accounts.
...
1. **Local bundled JSON** (`web/lib/localData.ts`) — `data/institutions.json`
   (private institutions, a static fixture sourced from `eduverify-api`) + a
   hand-maintained `web/lib/data/public_universities.json`/`public_tvets.json`
   (public universities/TVETs, which aren't in the DHET private-institution
   register at all). Always available, no network call, powers instant
   typeahead.
2. **DynamoDB** (`web/lib/dynamodb.ts`) — the live table, updated by
   `eduverify-api`'s ingestion pipeline. **Server-side only.** If it errors or
...
3. **`web/lib/keys.ts` (TypeScript, this repo) and `eduverify-api`'s
   `src/lib/keys.ts` (TypeScript, sibling repo) implement the *same*
   institution-keying logic independently.** If you change how an
   institution's key/slug is generated in one, you **must** change it in the
   other, or web lookups by ID will silently miss DynamoDB rows for affected
   institutions. There's no shared code between them — it's on you to keep
   them in sync by hand.
4. **`data/institutions.json`'s baked qualifications go stale relative to
   whatever produced it.** Its `faculties_and_programmes` field only reflects
   SAQA data as of the last time `npm run bake:faculties` (from `web/`) ran
   against it. If you pull a freshly-exported `data/institutions.json` from
   `eduverify-api`, re-run the bake step before treating the file as usable.
...
3. **Run the full suite**, not just your new test — regressions elsewhere are
   the whole point of this rule: `npm run test` (from `web/`).
...
- Do read `docs/DEPLOYMENT.md` and `terraform/CLAUDE.md` to understand the
  shape of it (the DynamoDB table `web/` reads, Amplify hosting, the CI/OIDC
  deploy role) before you're asked to touch it.
- Data ingestion into that DynamoDB table is `eduverify-api`'s job, not
  this repo's — there's no seed script or ingestion infra here to run.
...
3. Read one full `web/**/*.test.ts` file to learn this codebase's testing
   style before writing your first test.
...
- [`web/CLAUDE.md`](web/CLAUDE.md), [`terraform/CLAUDE.md`](terraform/CLAUDE.md) — per-part deep dives.
