...
`dynamo_item.to_item`/`institution_key` is the single source of truth for how an institution is keyed (`INST#<registration_number>`, or `INST#NAME#<slug>` when no registration number exists) — both `lambda_handler.py` and `scripts/seed_dynamodb.py` import it so live ingestion and bulk seeding key records identically. **`web/lib/keys.ts` reimplements the same slugify/key logic in TypeScript, and so does `eduverify-api`'s `src/lib/keys.ts` (a separate repo)** — if one changes, the other two must too, or lookups by ID will miss DynamoDB rows.
